...
1.  Client verifies cost >= minimum cost (3000 for 002, increases every year.)
...
1.  Client computes `pw`, `mk`, and `ak` using step (2) from Authentication Steps.

1.  Client registers with `email`, `pw`, `pw_cost`, `pw_salt`.
...
1.  Generate `string_to_auth` by combining the encryption version (002), the item's UUID, the IV, and the ciphertext using the colon ":" character:
